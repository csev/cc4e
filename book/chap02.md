...
arithmetic operators, so expressions like 
...
room to store it in the array `s`, so the test 

    i<lim-1 
    
_must_ be made first.
...
and logical expressions connected by `&&` and `||` are
...
truth value. For example, if `x` is 1 and `y` is 2, then 
...
which masks the last six bits of `x` to zero. Note that 

      x & ~077 

is independent of
word length, and is thus preferable to, for example, 
...
creates a mask with zeros in the rightmost `n` bits and
...
**Exercise 2-9.** In a 2's complement number system, 
